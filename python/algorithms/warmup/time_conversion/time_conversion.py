...
# A single string s containing a time in 12-hour clock format (i.e.: hh:mm:ss:AM or hh:mm:ss:PM), where 01 <= h <= 12 and 00 <= mm, ss <= 59.
...
def time_conversion(s):
  if s[-2] == 'A':
    ...
    if s[:2] == '12':
      return s[:len(s) - 2]
    return str(12 + int(s[:2])) + s[2:len(s) - 2]
